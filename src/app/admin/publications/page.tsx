...
//
// // ===================== PAGE =====================
// type TabKey = 'articles' | 'monographs' | 'chapters';
//
...
//
//     // ===== SEARCH po tytule (server-side) — osobno per tab =====
//     const [titleInput, setTitleInput] = useState<Record<TabKey, string>>({
//         articles: '',
//         monographs: '',
//         chapters: '',
//     });
//     const [titleQuery, setTitleQuery] = useState<Record<TabKey, string>>({
//         articles: '',
//         monographs: '',
//         chapters: '',
//     });
//
//     const activeTitleInput = titleInput[tab];
//     const activeTitleQuery = titleQuery[tab];
//
//     const setActiveTitleInput = (v: string) => setTitleInput((p) => ({ ...p, [tab]: v }));
//     const commitActiveTitleQuery = (v: string) => setTitleQuery((p) => ({ ...p, [tab]: v }));
//
//     // Modal szczegółów
...
//     }
//
//     async function loadList(page: number, overrides?: { disciplineId?: number; cycleId?: number; title?: string }) {
//         setItemsLoading(true);
//         setItemsError(null);
//
//         try {
//             const effectiveDisciplineId = toIntOr0(overrides?.disciplineId ?? disciplineId);
//             const effectiveCycleId = toIntOr0(overrides?.cycleId ?? cycleId);
//
//             // klucz: jeśli kliknęłaś "Szukaj", to overrides.title ma pierwszeństwo -> działa od razu (bez 2 kliknięcia)
//             const effectiveTitle = String(overrides?.title ?? activeTitleQuery ?? '').trim();
//
//             const body = {
//                 disciplineId: effectiveDisciplineId || null,
...
//         const t = String(activeTitleInput ?? '').trim();
//         commitActiveTitleQuery(t);
//         setPageMeta((p) => ({ ...p, page: 0 }));
//         void loadList(0, { title: t });
//     }
//
//     function resetAll() {
//         setDisciplineId(0);
//         setCycleId(0);
//
//         // reset tytułu tylko w aktywnej zakładce (bo ma być osobno per tab)
//         setTitleInput((p) => ({ ...p, [tab]: '' }));
//         setTitleQuery((p) => ({ ...p, [tab]: '' }));
//
//         setPageMeta((p) => ({ ...p, page: 0 }));
//         void loadList(0, { title: '' });
//     }
//
...
//         tab === 'articles' ? 'Szukaj po tytule artykułu…' : tab === 'monographs' ? 'Szukaj po tytule monografii…' : 'Szukaj po tytule rozdziału…';
//
//     const listCount = tab === 'articles' ? articles.length : tab === 'monographs' ? monographs.length : chapters.length;
//
...
//                             {activeTitleQuery.trim() ? `Brak wyników dla: "${activeTitleQuery.trim()}".` : 'Brak danych'}
//                         </div>
//                     ) : (
...
//                             {/* SEARCH server-side po tytule (osobno per tab) */}
//                             <div style={{ display: 'grid', gap: 6 }}>
//                 <span className={styles.muted} style={{ fontWeight: 800 }}>
//                   Szukaj po tytule (backend)
//                 </span>
//
//                                 <input
//                                     className={styles.searchInput}
...
//                                     <button className={styles.primaryBtn} onClick={applyTitleSearch} disabled={itemsLoading} style={{ flex: '1 1 auto' }}>
//                                         Szukaj
//                                     </button>
//                                     <button
...
//                                         onClick={() => {
//                                             setActiveTitleInput('');
//                                             commitActiveTitleQuery('');
//                                             setPageMeta((p) => ({ ...p, page: 0 }));
//                                             void loadList(0, { title: '' });
//                                         }}
//                                         disabled={itemsLoading && !activeTitleQuery.trim()}
...
//                                 </div>
//
//                                 {activeTitleQuery.trim() && (
//                                     <div className={styles.muted} style={{ fontSize: 12 }}>
//                                         Aktywne wyszukiwanie: <b>{activeTitleQuery.trim()}</b>
//                                     </div>
//                                 )}
//                             </div>
//
...
//                                     Zastosuj filtry
//                                 </button>
//                                 <button className={styles.ghostBtn} onClick={resetAll} disabled={itemsLoading} style={{ flex: '0 0 auto', whiteSpace: 'nowrap' }}>
//                                     Reset
//                                 </button>
//                             </div>
//
...
export default function AdminPublicationsPage() {
    ...
    // TITLE SEARCH — osobno per tab (input + wysłany query)
    const [titleInput, setTitleInput] = useState<Record<TabKey, string>>({
        articles: '',
        monographs: '',
        chapters: '',
    ...
    async function loadList(
        page: number,
        overrides?: { disciplineId?: number; cycleId?: number; title?: string }
    ) {
        setItemsLoading(true);
        setItemsError(null);
        ...
        try {
            ...
            const body = {
                // jeśli backend ma w AdminListRequest też id/typeId/sortBy — można je dodać jako null,
                // ale nie jest to konieczne jeśli nie są wymagane
                disciplineId: effectiveDisciplineId > 0 ? effectiveDisciplineId : null,
                cycleId: effectiveCycleId > 0 ? effectiveCycleId : null,
                page,
                size: pageMeta.size ?? 20,
                sortDir: 'DESC',
                title: effectiveTitle ? effectiveTitle : null,
    ...
    function applyFiltersServer() {
        const nextTitle = String(activeTitleInput ?? '').trim();

        // commit query dla aktywnego taba i od razu wykonaj request
        setTitleQuery((p) => ({ ...p, [tab]: nextTitle }));
        setPageMeta((p) => ({ ...p, page: 0 }));
        void loadList(0, { title: nextTitle });
    }

    function resetAllInCurrentTab() {
        setDisciplineId(0);
        setCycleId(0);
        ...
        setPageMeta((p) => ({ ...p, page: 0 }));
        void loadList(0, { disciplineId: 0, cycleId: 0, title: '' });
    }
    ...
    const titlePlaceholder =
        tab === 'articles'
            ? 'Szukaj po tytule artykułu…'
            : tab === 'monographs'
                ? 'Szukaj po tytule monografii…'
                : 'Szukaj po tytule rozdziału…';

    const visibleCount = tab === 'articles' ? articles.length : tab === 'monographs' ? monographs.length : chapters.length;
            ...
            <div className={styles.contentRow}>
                ...
                <div className={styles.rightColumn}>
                    ...
                    >
                        ...
                        <div style={{ display: 'grid', gap: 12 }}>
                            {/* Tytuł (backend) — BEZ osobnych przycisków "Szukaj/Wyczyść" */}
                            <div style={{ display: 'grid', gap: 6 }}>
                <span className={styles.muted} style={{ fontWeight: 800 }}>
                  Szukaj po tytule (backend)
                </span>
                                <input
                                    ...
                                    onChange={(e) => setActiveTitleInput(e.target.value)}
                                    onKeyDown={(e) => {
                                        if (e.key === 'Enter') applyFiltersServer();
                                    }}
                                />
                                <div className={styles.muted} style={{ fontSize: 12 }}>
                                    Wyszukiwanie wykona się po „Zastosuj filtry” lub Enter.
                                </div>
                            </div>
                            ...
                            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                                ...
                                <button className={styles.ghostBtn} onClick={resetAllInCurrentTab} disabled={itemsLoading} style={{ flex: '0 0 auto', whiteSpace: 'nowrap' }}>
                                    Reset
                                </button>
                            </div>

                            <div className={styles.muted} style={{ fontSize: 12 }}>
                                Sortowanie w backendzie: domyślnie DESC.
                            </div>
                        </div>
                    </div>
                </div>
