                    ...
                    {loading ? (
                        ...
                        <div className={styles.cardsGrid}>
                            {items.map((it) => {
                                ...
                                const disciplineId = Number(it?.discipline?.id ?? 0) || 0;
                                        ...
                                        <div className={styles.cardBottom} style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}>
                                            ...
                                            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                                                ...
                                                <button
                                                    ...
                                                    onClick={() => addChapterToSlots(id, disciplineId)}
                                                    disabled={!id || !disciplineId || slotBusyId === id}
                                                    title={!disciplineId ? 'Brak dyscypliny w danych rozdziału' : 'Dodaj do slotów'}
                                                >
                ...
                {modalLoading ? (
                    ...
                    <>
                        <div className={styles.kvGrid} style={{ marginBottom: 12, overflow: 'visible' }}>
                            ...
                            <div className={styles.kvVal}>
                                <input className={styles.searchInput} value={String(draft.monograficChapterTitle ?? '')} onChange={(e) => setDraft((p: any) => ({ ...p, monograficChapterTitle: e.target.value }))} />
                            </div>

                            <div className={styles.kvKey}>Tytuł monografii</div>
                            <div className={styles.kvVal}>
                                <input className={styles.searchInput} value={String(draft.monograficTitle ?? '')} onChange={(e) => setDraft((p: any) => ({ ...p, monograficTitle: e.target.value }))} />
                            </div>

                            <div className={styles.kvKey}>Wydawca</div>
                            <div className={styles.kvVal}>
                                <input className={styles.searchInput} value={String(draft.monographPublisher ?? '')} onChange={(e) => setDraft((p: any) => ({ ...p, monographPublisher: e.target.value }))} />
                            </div>
                        ...
                        <CoauthorsPicker value={Array.isArray(draft.coauthor) ? draft.coauthor : []} onChange={(next) => setDraft((p: any) => ({ ...p, coauthor: next }))} label="Współautorzy" />
                    ...
                    {coauthorLabel(c)}
                        ...
                        <div style={{ display: 'flex', gap: 10, marginTop: 12, flexWrap: 'wrap' }}>
                            ...
                            <button
                                className={styles.primaryBtn}
                                onClick={() => addChapterToSlots(Number(draft.id), Number(draft.disciplineId ?? 0))}
                                disabled={!Number(draft?.id) || !Number(draft?.disciplineId) || slotBusyId === Number(draft?.id)}
                                title={!Number(draft?.disciplineId) ? 'Najpierw ustaw dyscyplinę' : 'Dodaj do slotów'}
                            >
                                {slotBusyId === Number(draft?.id) ? 'Dodawanie…' : 'Dodaj do slotów'}
                            </button>
