    ...
    // ===== ADMIN (SŁOWNIKI: GET-with-body przez proxy admin) =====
    ...
    function mapApiError(status: number, rawText: string): string {
        ...
        // Spring / walidacje – różne możliwe formaty
        ...
        async function updatePublication() {
            ...
            try {
                const body = {
                    ...
                    publicationYear: draft.publicationYear != null && String(draft.publicationYear).trim() !== ''
                        ? Number(draft.publicationYear)
                        : null,

                    // tylko {userId, fullName} (bez unitName)
                    replaceCoauthors: Array.isArray(draft.replaceCoauthors)
                        ? draft.replaceCoauthors.map((c: any) => ({
                ...
                const res = await authFetch(`/api/article/worker/updatePublication?publicationId=${publicationId}`, {
                    method: 'PATCH',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(body),
                } as RequestInit);
                ...
                // odśwież draft (żeby modal pokazał aktualne dane)
                ...
                <div className={styles.bigCardFull} style={{ marginTop: 16 }}>
                    ...
                    <form onSubmit={createPublication} style={{ display: 'grid', gap: 10, marginTop: 12 }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 10 }}>
                            ...
                            <input className={styles.searchInput} placeholder="journalTitle" value={createForm.journalTitle} onChange={(e) => setCreateForm((p) => ({ ...p, journalTitle: e.target.value }))} />
                        </div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 10 }}>
                            ...
                            <input className={styles.searchInput} placeholder="publicationYear" value={createForm.publicationYear} onChange={(e) => setCreateForm((p) => ({ ...p, publicationYear: e.target.value }))} />
                        </div>

                        <CoauthorsPicker
                            value={createForm.coauthors}
                            onChange={(next) => setCreateForm((p) => ({ ...p, coauthors: next }))}
                            label="Coautorzy (szukaj po fullName; userId=0 = inna uczelnia)"
                        />
                    ...
                    {modalLoading ? (
                        ...
                        <>
                            <div className={styles.kvGrid} style={{marginBottom: 12}}>
                                ...
                                <div className={styles.kvVal}>
                                    <select
                                        ...
                                        onChange={(e) => setDraft((p: any) => ({
                                            ...p,
                                            disciplineId: toIntOr0(e.target.value)
                                        }))}
                                        disabled={filtersLoading}
                                    >
                                        {disciplineOptions.map((d) => (
                                ...
                                <div className={styles.kvVal}>
                                    <input className={styles.searchInput} value={String(draft.title ?? '')}
                                           onChange={(e) => setDraft((p: any) => ({...p, title: e.target.value}))}/>
                                </div>

                                <div className={styles.kvKey}>Rok</div>
                                <div className={styles.kvVal}>
                                    <input className={styles.searchInput} value={String(draft.publicationYear ?? '')}
                                           onChange={(e) => setDraft((p: any) => ({
                                               ...p,
                                ...
                                <div className={styles.kvVal}>
                                    <input
                                        ...
                                        value={String(draft.issn ?? '')}
                                        onChange={(e) => setDraft((p: any) => ({...p, issn: e.target.value}))}
                                    />
                                </div>
                                ...
                                <div className={styles.kvVal}>
                                    <input
                                        className={styles.searchInput}
                                        value={String(draft.eissn ?? '')}
                                        onChange={(e) => setDraft((p: any) => ({...p, eissn: e.target.value}))}
                                    />
                                </div>
                                <div className={styles.kvKey}>Czasopismo</div>
                                <div className={styles.kvVal}>
                                    <input className={styles.searchInput} value={String(draft.journalTitle ?? '')}
                            ...
                            <CoauthorsPicker
                                ...
                                label="Coautorzy (replaceCoauthors; userId=0 = inna uczelnia)"
                            />
