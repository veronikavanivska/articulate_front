...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const target = `${API_GATEWAY.replace(/\/$/, '')}/etl/admin/listMeinVersions${qs ? `?${qs}` : ''}`;
        console.log('[proxy/listMeinVersions] ->', target);
        ...
        if (ct.includes('application/json')) {
            try {
                ...
                return res.json({ ...json, items, item: items });
            } catch {
        ...
        console.error('[proxy/listMeinVersions] error', err);
