...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const target = `${API_GATEWAY.replace(/\/$/, '')}/etl/admin/deleteMeinMonoVersion${qs ? `?${qs}` : ''}`;
        console.log('[proxy/deleteMeinMonoVersion] ->', target);
        ...
        console.error('[proxy/deleteMeinMonoVersion] error', err);
