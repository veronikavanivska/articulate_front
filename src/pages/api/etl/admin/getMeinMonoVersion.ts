...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const target = `${API_GATEWAY.replace(/\/$/, '')}/etl/admin/getMeinMonoVersion${qs ? `?${qs}` : ''}`;
        console.log('[proxy/getMeinMonoVersion] ->', target);
        ...
        console.error('[proxy/getMeinMonoVersion] error', err);
