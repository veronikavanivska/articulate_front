...
// POST na proxy -> GET z body na backend (bo masz @GetMapping + @RequestBody)
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        // API Gateway: /slots/<...>
        const backendUrl = `${backendBase}/slots/${parts.map(encodeURIComponent).join('/')}${qs ? `?${qs}` : ''}`;

        const bodyStr =
            req.body == null ? '' : typeof req.body === 'string' ? req.body : JSON.stringify(req.body);

        // hack: POST na proxy -> GET z body na backend (dla getSlots)
