...
// Jeśli kiedyś będziesz mieć endpointy "GET-with-body", dopisz ich leaf nazwy tutaj
// (jak w Twoim przykładzie). Na ten moment zostawiam puste.
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    // Nagłówki jak w Twoim przykładzie
    ...
    try {
        ...
        const backendUrl = `${backendBase}/profile/${parts.map(encodeURIComponent).join('/')}${qs ? `?${qs}` : ''}`;

        const bodyStr = req.body == null ? '' : typeof req.body === 'string' ? req.body : JSON.stringify(req.body);

        // opcjonalne "POST -> GET z body" (tylko gdy potrzebujesz)
